...
func (d *metalDriver) applyIPAddresses(ctx context.Context, req *driver.CreateMachineRequest, providerSpec *apiv1alpha1.ProviderSpec) ([]map[string]any, error) {
	...
	for _, networkRef := range providerSpec.IPAMConfig {
		...
		if networkRef.IPAMRef != nil && networkRef.IPAMRef.APIGroup == capiv1beta1.GroupVersion.Group {
			addressMetaData, err := d.applyCapiIPAddress(ctx, networkRef, ipAddrKey)
			if err != nil {
				return nil, err
			}
...
func (d *metalDriver) applyCapiIPAddress(ctx context.Context, networkRef apiv1alpha1.IPAMConfig, ipAddrKey apitypes.NamespacedName) (map[string]any, error) {
	ipClaim := &capiv1beta1.IPAddressClaim{}
	if err := d.metalClient.Get(ctx, ipAddrKey, ipClaim); err != nil && !apierrors.IsNotFound(err) {
		return nil, err
	} else if err == nil {
		klog.V(3).Infof("IP found %s", ipAddrKey.String())
